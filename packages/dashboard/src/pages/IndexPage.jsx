...
export default function IndexPage() {
  ...
  const { serviceMetrics, financialMetrics, expenseMetrics } = useMemo(() => {
    ...
    const { serviceByYear, yearsWithFullServiceData } =
      buildServiceByYear(serviceData);
    const expensesByYear = buildExpensesByYear(financialData.expenses);
    const expensePerHouseholdYears = getExpensePerHouseholdYears(
      expensesByYear,
      yearsWithFullServiceData,
    );

    let totalExpForMetrics = 0,
      totalHouseholdsForMetrics = 0,
      totalIndividualsForMetrics = 0;
    ...
    const avgPerHousehold =
      totalHouseholdsForMetrics > 0
        ? totalExpForMetrics / totalHouseholdsForMetrics
        : 0;
    const avgPerIndividual =
      totalIndividualsForMetrics > 0
        ? totalExpForMetrics / totalIndividualsForMetrics
        : 0;
    ...
    return {
      serviceMetrics: [
        {
          value: serviceTotals.households.toLocaleString(),
          label: "Households served",
        },
        {
          value: serviceTotals.people.toLocaleString(),
          label: "People served",
        },
        {
          value: serviceTotals.volunteerHours.toLocaleString(),
      ...
      expenseMetrics: [
        {
          value: formatCurrency(avgPerHousehold),
          label: "Expense per household",
        },
        {
          value: formatCurrency(avgPerIndividual),
          label: "Expense per individual",
        },
        {
          value: totalHouseholdsForMetrics.toLocaleString(),
          label: "Households (full-year data)",
        },
  ...
  return (
    <div class="grid">
      <Card title="Service summary" fullWidth>
        ...
        <a href="#/service-report" class="report-link">
          View Service Report →
        </a>
      </Card>
      <Card title="Financial summary" fullWidth>
        <Metrics items={financialMetrics} />
        <a href="#/financials" class="report-link">
      ...
      >
        ...
        <a href="#/service-expenses" class="report-link">
          View Expense per Household & Individual →
        </a>
      </Card>
    </div>
  );
