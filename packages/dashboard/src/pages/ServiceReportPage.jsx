...
export default function ServiceReportPage() {
  const {
    ...
    loading: dataLoading,
    error: dataError,
  } = useFetchJson("./data/service-report.json");
  ...
  const years = useMemo(
    () => [...new Set(sortedData.map((r) => r.year))].sort(),
    [sortedData],
  );
  ...
  const metrics = useMemo(() => {
    ...
    return [
      ...
      {
        value: totals.volunteerHours.toLocaleString(),
  ...
  const chartConfigs = useMemo(() => {
    ...
    const seniorsPerRow = (r) => {
      ...
      return (
        (s.households_without_children ?? 0) + (s.households_with_children ?? 0)
      );
    };
    ...
    const ageLabels = {
      "infants:_0_-_5": "0–5",
      between_6_and_12: "6–12",
      between_13_and_27: "13–17",
      between_18_and_30: "18–30",
      between_31_and_45: "31–45",
      between_46_and_59: "46–59",
      between_60_and_74: "60–74",
      "75_and_over": "75+",
      unknown: "Unknown",
    };
    filtered.forEach((r) => {
    ...
    const sexEntries = Object.entries(sexCounts)
      .filter(([, v]) => v > 0)
      .sort((a, b) => b[1] - a[1]);
    ...
    const ethnicEntries = Object.entries(ethnicCounts)
      .filter(([, v]) => v > 0)
      .sort((a, b) => b[1] - a[1]);
    ...
    return {
      ...
      population: {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Adults",
              data: filtered.map((r) => r.client_types?.adults ?? 0),
              borderColor: "#2563eb",
            ...
            {
              label: "Children",
              data: filtered.map((r) => r.client_types?.children ?? 0),
              borderColor: "#7c3aed",
            ...
            {
              label: "Seniors",
              data: filtered.map(seniorsPerRow),
              borderColor: "#0891b2",
          ...
          datasets: [
            {
              label: "First visit",
              data: filtered.map(
                (r) => r.client_visit_frequency?.first_time ?? 0,
              ),
              borderColor: "#2563eb",
              backgroundColor: "rgba(37, 99, 235, 0.1)",
              fill: true,
            ...
            },
            {
              label: "Second visit",
              data: filtered.map(
                (r) => r.client_visit_frequency?.second_time ?? 0,
              ),
              borderColor: "#7c3aed",
              backgroundColor: "rgba(124, 58, 237, 0.1)",
              fill: true,
            ...
            },
            {
              label: "Third or more",
              data: filtered.map(
                (r) => r.client_visit_frequency?.third_or_more_time ?? 0,
              ),
              borderColor: "#0891b2",
          ...
          datasets: [
            {
              ...
              data: filtered.map((r) => {
                ...
                return h > 0
                  ? Number(((r.volunteerhours ?? 0) / h).toFixed(1))
                  : 0;
              }),
              borderColor: "#2563eb",
              backgroundColor: "rgba(37, 99, 235, 0.1)",
          ...
          datasets: [
            {
              ...
              data: filtered.map((r) => {
                const h = r.client_types?.households ?? 0;
                return h > 0
                  ? Number(((r.client_types?.total ?? 0) / h).toFixed(1))
                  : 0;
              }),
              borderColor: "#7c3aed",
              backgroundColor: "rgba(124, 58, 237, 0.1)",
          ...
          datasets: [
            {
              ...
              data: filtered.map(
                (r) => r.number_of_households?.households_without_children ?? 0,
              ),
            ...
            },
            {
              label: "With children",
              data: filtered.map(
                (r) => r.number_of_households?.households_with_children ?? 0,
              ),
      ...
      visits: {
        ...
        data: {
          ...
          datasets: [
            {
              data: [vf.first, vf.second, vf.third],
      ...
      sex: {
        ...
        data: {
          ...
          datasets: [
            {
              data: sexEntries.map(([, v]) => v),
              backgroundColor: ["#2563eb", "#7c3aed", "#0891b2", "#94a3b8"],
            },
          ],
        },
        options: chartDefaults,
      ...
      ethnicity: {
        type: "doughnut",
        data: {
          labels: ethnicEntries.map(
            ([k]) => ethnicLabels[k] ?? k.replace(/_/g, " "),
          ),
          datasets: [
            {
              data: ethnicEntries.map(([, v]) => v),
              backgroundColor: [
                "#2563eb",
                "#7c3aed",
                "#0891b2",
  ...
  const tableRows = useMemo(() => {
    return filtered.map((r) => {
      ...
      return [
        ...
        h.toLocaleString(),
        p.toLocaleString(),
        (r.volunteerhours ?? 0).toLocaleString(),
        r.operatingdays ?? 0,
        ppH,
      ];
    });
