...
export default function FinancialsPage() {
  const {
    ...
    loading: dataLoading,
    error: dataError,
  } = useFetchJson("./data/financials.json");
  ...
  const filtered = useMemo(() => {
    ...
    const filter = (rows) =>
      (rows ?? []).filter((r) => r.year >= start && r.year <= end);
    return {
  ...
  const chartConfigs = useMemo(() => {
    ...
    const incCatEntries = Object.entries(incByCat)
      .filter(([, v]) => v > 0)
      .sort((a, b) => b[1] - a[1]);
    ...
    const expCatEntries = Object.entries(expByCat)
      .filter(([, v]) => v > 0)
      .sort((a, b) => b[1] - a[1]);
    ...
    const topIncome = Object.entries(incBySource)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 12);
    ...
    const topExpense = Object.entries(expBySource)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 12);
    ...
    (filtered.income ?? []).forEach((r) => {
      ...
      const amt = Number(r.amount) || 0;
      incByYearCat[r.year][r.category] =
        (incByYearCat[r.year][r.category] || 0) + amt;
    });
    const incCategories = [
      ...new Set((filtered.income ?? []).map((r) => r.category)),
    ].filter(Boolean);
    ...
    (filtered.expenses ?? []).forEach((r) => {
      ...
      const amt = Number(r.amount) || 0;
      expByYearCat[r.year][r.category] =
        (expByYearCat[r.year][r.category] || 0) + amt;
    });
    const expCategories = [
      ...new Set((filtered.expenses ?? []).map((r) => r.category)),
    ].filter(Boolean);
    ...
    (filtered.income ?? []).forEach((r) => {
      ...
      const amt = Number(r.amount) || 0;
      incByYearSource[r.year][r.source] =
        (incByYearSource[r.year][r.source] || 0) + amt;
    });
    const trendIncColors = [
      "#059669",
      "#047857",
      "#065f46",
      "#134e4a",
    ...
    (filtered.expenses ?? []).forEach((r) => {
      ...
      const amt = Number(r.amount) || 0;
      expByYearSource[r.year][r.source] =
        (expByYearSource[r.year][r.source] || 0) + amt;
    });
    const trendExpColors = [
      "#cc5c5c",
      "#b94a9e",
      "#9b5cc5",
      "#8554c4",
    ...
    for (let i = 1; i < years.length; i++) {
      ...
      yoyIncome.push(
        prevInc > 0 ? Math.round(((inc - prevInc) / prevInc) * 100) : 0,
      );
      yoyExpense.push(
        prevExp > 0 ? Math.round(((exp - prevExp) / prevExp) * 100) : 0,
      );
    }
              ...
              callbacks: {
                afterBody: (items) => {
                  ...
                  if (idx != null && netData[idx] != null) {
                    const n = netData[idx];
                    return n >= 0
                      ? `Net: +$${n.toLocaleString()}`
                      : `Net: -$${Math.abs(n).toLocaleString()}`;
                  }
                  return "";
                },
              },
          ...
          datasets: [
            {
              label: "Net margin %",
              data: netMarginData,
              backgroundColor: netMarginData.map((v) =>
                v >= 0 ? "rgba(5, 150, 105, 0.8)" : "rgba(185, 74, 158, 0.8)",
              ),
              hoverBackgroundColor: netMarginData.map((v) =>
                v >= 0 ? "rgba(5, 150, 105, 1)" : "rgba(185, 74, 158, 1)",
              ),
            },
          ],
        },
        ...
        data: {
          ...
          datasets: [
            ...
            },
            {
              label: "Expenses",
              data: yoyExpense,
              backgroundColor: "rgba(185, 74, 158, 0.7)",
              hoverBackgroundColor: "rgba(185, 74, 158, 1)",
            },
          ],
        },
        options: {
  ...
  const hasData =
    (filtered.income?.length ?? 0) > 0 || (filtered.expenses?.length ?? 0) > 0;
